...
%%
sol_pre = [ 1.6469  3.2183  0.3562 -2.4252 -0.773  -2.2135];
py_sol = [-4.5256  3.1779  0.4337 -2.4336 -0.7894 -2.3423];
mat_sol = [-1.6360   -0.1401   -0.2527   -0.5884    2.2323   -2.0461];
...
path_world = [-0.30000000000000004, 0.6000000000000001, 0.2;
                -0.30000000000000004, 0.5, 0.3;
                -0.30000000000000004, 0.5, 0.4;
                -0.30000000000000004, 0.40000000000000013, 0.5;
                -0.30000000000000004, 0.40000000000000013, 0.6;
                -0.30000000000000004, 0.40000000000000013, 0.7;
                -0.30000000000000004, 0.30000000000000004, 0.7999999999999999;
                -0.30000000000000004, 0.19999999999999996, 0.7999999999999999;
                -0.30000000000000004, 0.10000000000000009, 0.8999999999999999;
                -0.30000000000000004, 0.0, 0.8999999999999999;
...
Step 0: q = [-1.3162  0.7031 -2.0751  1.637   1.875  -1.9533]
Step 1: q = [-1.2651  0.546  -2.2078  1.7773  1.908  -2.1288]
Step 2: q = [-1.2756  0.3727 -2.156   1.7521  1.8544 -2.3116]
Step 3: q = [-1.2163  0.1502 -2.1877  1.8697  1.8459 -2.4925]
Step 4: q = [-1.2437 -0.0075 -2.0786  1.8007  1.7307 -2.6494]
Step 5: q = [-1.2771 -0.1527 -1.9482  1.7139  1.5948 -2.7853]
Step 6: q = [-1.2216 -0.3642 -1.8573  1.749   1.5269 -2.9451]
Step 7: q = [-1.126  -0.4397 -1.9204  1.826   1.4797 -3.1246]
Step 8: q = [-2.4226 -1.0953 -1.8478  1.3737  0.561  -1.8339]
Step 9: q = [-2.0187 -1.0878 -1.7762  1.6474  0.5991 -2.4822]
Step 10: q = [-1.7913 -1.15   -1.7148  1.763   0.592  -2.8318]
Step 11: q = [-1.6841 -1.2579 -1.6364  1.7983  0.5172 -3.0246]
Step 12: q = [-1.6316 -1.3652 -1.7257  1.9608  0.4061  3.1306]
Step 13: q = [-1.6047 -1.5086 -1.5925  1.9141  0.277   3.0387]
Step 14: q = [-1.5915 -1.6668 -1.6019  1.9819  0.1389  2.9965]
Step 15: q = [-1.5893 -1.4663 -1.7984  3.2647 -0.0185  1.5708]
...
% hWaitBar = waitbar(0, 'Calculando configuraciones articular y voxeles...', 'Name', 'Progreso', 'WindowStyle', 'modal');

% Iterar sobre todas las configuraciones articulares
iteration_count = 0;
for q1_i = joint_min:joint_step:joint_max
    fprintf('INFO: Nuevo valor de q1 \n')
    for q2_i = joint_min:joint_step:joint_max
        fprintf('INFO: Nuevo valor de q2 \n')
        for q3_i = joint_min:joint_step:joint_max
            fprintf('INFO: Nuevo valor de q3 \n')
            for q4_i = joint_min:joint_step:joint_max
                % fprintf('INFO: Nuevo valor de q4 \n')
                for q5_i = joint_min:joint_step:joint_max
                    ...
                    if x >= cart_min && x <= cart_max && y >= cart_min && y <= cart_max && z >= cart_min && z <= cart_max
                        ...
                        voxels{ix, iy, iz} = [voxels{ix, iy, iz}; q_current, quaternion];
                    else
                    ...
                    % waitbar(iteration_count / total_iterations, hWaitBar);

                    % Mostrar el progreso en la consola
                    iteration_count = iteration_count + 1;
                    if mod(iteration_count, round(total_iterations / 10)) == 0  % Cada 10% del progreso
...
% % Cargar la información de los voxeles desde el archivo .mat
